...
4. Run `GITHUB_AUTH=xxx ./bin/publish` from the root of the monorepo. Just like with the Automated Publishing Path, if you are on the `master` branch, a new alpha release will be created, and if you are on the `stable` branch, a new mainline release.
