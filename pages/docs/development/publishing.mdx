...
We use a combination of tools from `lerna` and `npm` to handle publishing these packages. A detailed blog post on our publishing process can be found [here](/blog/grouparoo-monorepo-deployment).
...
4. Run `GITHUB_AUTH=xxx ./bin/publish`
