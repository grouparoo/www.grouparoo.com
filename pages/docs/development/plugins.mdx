...
If your Plugin is providing apps or connections you can use the `plugin.registerPlugin` method. See the simplified example below. You can see the full file from [`@grouparoo/postgres` here](https://github.com/grouparoo/grouparoo/blob/master/plugins/%40grouparoo/postgres/src/initializers/plugin.ts)
