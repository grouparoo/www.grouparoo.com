...
date: "2021-01-05"
section: "development"
pullQuote: "Build a source to import import data into Grouparoo customer profiles."
---
...
The properties of a source `Connection` are:
...
- **Property Methods**
  ...
  - `profileProperty: ProfilePropertyPluginMethod`: Given a profile and the property definition, get the value of the property for that profile.
  - `profileProperties: ProfilePropertiesPluginMethod`: Given an array of profiles and the property definition, get the value of the property for each profile.

- **Schedule Methods**

  These methods are used when the source can have a schedule.

  - `profiles: ProfilesPluginMethod`: This method is the core method to implement a schedule. It uses a high-water mark and given options to return a collection of profiles to import. This options might also return properties to set on the profiles The main goal of this method is to call `await plugin.createImport(schedule, run, row)` for each row of data, where row is an object like `{user_id: 9, email: "person@example.com", first_name: "person"}`
