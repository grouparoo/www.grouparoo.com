...
The properties of a destination `Connection` are:
...
- **Data Methods**
  ...
  - `profileProperty: ExportProfilePluginMethod` Given the property values and group memberships to export for a single profile, send it to the destination. The singular case is much easier to reason about, particularly when there is an error with the destination.
  - `profileProperties: ExportProfilesPluginMethod` Given the property values and group memberships to export for many profiles, send it to the destination. The extra semantics can be more complicated because there can be partial failures. However, if the destination has a "batch" API, it's generally better to use this for performance reasons.

When defining `methods`, link to the method directly, do not use a string name.

## Research

The first thing to do is create a document that discusses how the Grouparoo model (Profiles with properties, Group memberships) map to the destination system. Here are some example from [Salesforce](https://github.com/grouparoo/grouparoo/issues/649) and [Intercom](https://github.com/grouparoo/grouparoo/issues/1046).
...
- Anything to learn from other integrations?
  - [Segment](https://segment.com/docs/connections/destinations/catalog/)? _Only handles Users_
  - [Lytics](https://www.lytics.com/integrations/)?
...
- Salesforce
  Salesforce has basically unlimited mappings with some primary core "user" Object models: Lead and Contact. They could also be literally anything. A common group mapping is to Campaigns, but it can also be very generic. We decided to start with the generic case. There is a highly configurable destination where you pick the Salesforce Object that you want to sync with. We will likely make more targeted destinations (Leads/Contacts in Campaigns) as we learn more.
...
The easiest way to get started is to pick the approach and copy an existing plugin. Pick the ones that is closest. For example if you are making another ad platform destination that does batching, [facebook](https://github.com/grouparoo/grouparoo/tree/master/plugins/%40grouparoo/facebook) might be a good bet. Otherwise, here are some simple defaults.

- Single profile at a time (`exportProfile`): [hubspot](https://github.com/grouparoo/grouparoo/tree/master/plugins/%40grouparoo/hubspot)
- Batch multiple profiles (`exportProfiles`): [marketo](https://github.com/grouparoo/grouparoo/tree/master/plugins/%40grouparoo/marketo)

After copying, change the names in the `plugin.ts`, `package.json`, and the directory structure to the one you are making. Look for an icon with a license that we can use and not the attribution in the `README.md`. The icon should be square and 256+ pixels. White background are ok.
...
- Defined
  A common case is that the system has a static (or default) set of fields, but the user can add more in the system settings. In this case, an API call is needed to fetch the list. In these cases, they are then mapping to Grouparoo types from the type given in the API request like in [marketo](https://github.com/grouparoo/grouparoo/blob/c525ba5a5bcc1dfbab848991ea0475fa7fd5cb86/plugins/@grouparoo/marketo/src/lib/export/destinationMappingOptions.ts#L33-L63). The `destinationMappingOptions` method allows some `known` fields to be marked as `important`. This is done for the most likely ones that a Grouparoo user would want (first name, for example). These are presented immediately in the UI. The rest show up in a typeahead. The [marketo](https://github.com/grouparoo/grouparoo/blob/c525ba5a5bcc1dfbab848991ea0475fa7fd5cb86/plugins/@grouparoo/marketo/src/lib/export/destinationMappingOptions.ts#L65-L69) mapping does this by name.
- Dynamic
