...
- **Data Methods**
  ...
  - `profileProperty `[`ExportRecordPluginMethod`](https://docs.grouparoo.com/interfaces/exportrecordpluginmethod.html) Given the property values and group memberships to export for a single profile, send it to the destination. The singular case is much easier to reason about, particularly when there is an error with the destination.
  - `profileProperties `[`ExportRecordsPluginMethod`](https://docs.grouparoo.com/interfaces/exportrecordspluginmethod.html) Given the property values and group memberships to export for many profiles, send it to the destination. The extra semantics can be more complicated because there can be partial failures. However, if the destination has a "batch" API, it's generally better to use this for performance reasons.
...
- Single profile at a time (`exportRecord`): [hubspot](https://github.com/grouparoo/grouparoo/tree/main/plugins/%40grouparoo/hubspot)
- Batch multiple profiles (`exportRecords`): [marketo](https://github.com/grouparoo/grouparoo/tree/main/plugins/%40grouparoo/marketo)
...
The core code of the destination plugin is the `exportRecord` (single) or `exportRecords` (batch) method. It is given the (old and new) properties and the (old and new) group membership. If the profile is to be deleted, that is noted. The goal is to sync that information to the destination system.
...
- **Find existing users**
  ...
  See if the user(s) already exist in the system. This often uses a combination of the old and new profile properties. For example, [intercom](https://github.com/grouparoo/grouparoo/blob/a24a6b2425eca2947466ae041efc5b83a876e87b/plugins/@grouparoo/intercom/src/lib/export-users/exportRecord.ts#L173-L252) uses all the primary key data to search for a user and then prioritizes the response.
...
Plugins are tested using [jest](https://jestjs.io/). The primary way of testing them is to test the individual methods (`exportRecord`, `destinationMappingOptions`, etc) in their own file/suite. Several plugins have multiple suites for exporting profiles when there are `destinationOptions` that make them work differently.
