...
- **Verify Data**
  ...
  Create an error if a required data (in `newRecordProperties` is not present). For example, [zendesk](https://github.com/grouparoo/grouparoo/blob/c525ba5a5bcc1dfbab848991ea0475fa7fd5cb86/plugins/@grouparoo/zendesk/src/lib/export/exportProfile.ts#L23-L27) throws an error if there is no external id.

- **Find existing users**

  See if the user(s) already exist in the system. This often uses a combination of the old and new record properties. For example, [intercom](https://github.com/grouparoo/grouparoo/blob/a24a6b2425eca2947466ae041efc5b83a876e87b/plugins/@grouparoo/intercom/src/lib/export-users/exportProfile.ts#L173-L252) uses all the primary key data to search for a user and then prioritizes the response.
...
- **Create or Update the user**
  ...
  Build a payload from the given property values. Use the `oldRecordProperties` that are not in `newRecordProperties` to known which values should be cleared. This sometimes means setting it to `null` or `undefined` like in [zendesk](https://github.com/grouparoo/grouparoo/blob/c525ba5a5bcc1dfbab848991ea0475fa7fd5cb86/plugins/@grouparoo/zendesk/src/lib/export/exportProfile.ts#L60-L75) or an empty string like in [malichimp](https://github.com/grouparoo/grouparoo/blob/c525ba5a5bcc1dfbab848991ea0475fa7fd5cb86/plugins/%40grouparoo/mailchimp/src/lib/shared/updateProfile.ts#L50-L57). Sometimes, it means setting a default value like in [salesforce](https://github.com/grouparoo/grouparoo/blob/c525ba5a5bcc1dfbab848991ea0475fa7fd5cb86/plugins/%40grouparoo/salesforce/src/lib/export/salesforceExporter.ts#L309-L335).
...
It's important to be sure to clean up written records so that the first one in the test really is a new user in the destination system. For example [marketo](https://github.com/grouparoo/grouparoo/blob/c525ba5a5bcc1dfbab848991ea0475fa7fd5cb86/plugins/@grouparoo/marketo/__tests__/integration/export-profiles.ts#L81-L83) cleans up all test users and lists at the start and end of every run of the suite.
