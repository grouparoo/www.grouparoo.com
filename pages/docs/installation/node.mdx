...
By default, Grouparoo will run from a SQLite database and in-memory Redis, so no external dependencies are required. However, if you want to safely persists data between runs, you'll need to also install Postgres and Redis.
...
Finally, create a Postgres database for Grouparoo to use with `createdb grouparoo_development`.
