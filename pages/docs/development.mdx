...
Options:
  ...
  -c, --config [config]          add flag to write to config directory and not populate configuration into Grouparoo database
  --reset [reset]                only clear Grouparoo database and don't load config
  --seed [seed]                  add flag to only write (or output) demo source data and not touch Grouparoo database
  --setup [setup]                only create the login Team Member
  --b2c [b2c]                    (default) loads users and admins models
  -b, --b2b [b2b]                loads account model
  -u, --users [users]            includes users model with purchases
  -a, --accounts [accounts]      includes accounts model with with payments
  --admins [admins]              includes admins model
  --postgres [postgres]          (default) load source data into local Postgres database
  --mongo [mongo]                load specified source data into local MongoDB database
  --mysql [mysql]                load specified source data into local MySQL database
  --snowflake [snowflake]        assumes a Snowflake instance with data already present
  --bigquery [bigquery]          assumes a BigQuery instance with data already present
  -m, --mailchimp [mailchimp]    create Mailchimp destination for data
  -s, --salesforce [salesforce]  create Salesforce destination for data
...
A note on chromedriver for OSX Users: As Chrome updates, you may need to `brew reinstall --cask chromedriver` to keep up to date as well. Every time you update chromedriver, you will need to re-authorize the application to run in system preferences, as it is unsigned.
