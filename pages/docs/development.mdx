...
docker run -d --rm --name grouparoo-postgres -p 5432:5432 -e POSTGRES_HOST_AUTH_METHOD=trust -e POSTGRES_DB=grouparoo_development postgres
```
...
# use the postgres official Docker image:
docker run -d --rm --name grouparoo-postgres -p 4521:5432 -e POSTGRES_HOST_AUTH_METHOD=trust -e POSTGRES_DB=grouparoo_development postgres
```
