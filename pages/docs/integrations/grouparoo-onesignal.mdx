...
OneSignal allows you to set up multiple apps on their platform, think of a OneSignal App as a configuration for sending communications via push notifications, in-app messages, SMS, or email. To find the App ID and App Key you’ll need for the first steps of Grouparoo's configuration, navigate on OneSignal to the specific App where you want to sync data. Choose _Settings_, then _Keys & IDs_ from the top navigation bar. Treat your API key like a password and be sure to keep it private. Anyone with access to this key would be able to send notifications from your OneSignal app.
