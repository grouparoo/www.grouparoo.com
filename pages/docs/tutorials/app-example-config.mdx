...
To see a list of available Generators at any time, use the `--list` flag.
    ...
    Available Templates:
      ...
      group (id) - Config for a Grouparoo Group
...
Let's generate those config objects, beginning with the `all_emails` Group:
    ...
    $ grouparoo generate group all_emails
...
Then let's do the same for `high_value_spanish_speakers`:
    ...
    $ grouparoo generate group high_value_spanish_speakers
  ...
  return [
    {
      ...
      // Include Records with an `ltv` above 100 who have a value of `Spanish`
      // for `language`.
      rules: [
        {
          propertyId: "ltv",
