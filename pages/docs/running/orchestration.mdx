...
Grouparoo can always be run via the CLI via `grouparoo run`. Some customers may want to run Grouparoo via the CLI directly from their orchestrator a the proper time at the end of their ETL/ELT pipeline. For these use cases, we recommend configuring Grouparoo via code config and using the `grouparoo run` command. This will run all Schedules at this time, when `grouparoo run` is started.

There are [flags which can be used to customize the run command's behavior](/docs/cli/run#run). This method of triggering Grouparoo is only applicable when self-hosting Grouparoo.
...
The `/api/v1/schedules/run` endpoint can be used to trigger a Run for your Schedules. You can run all of them at once or provide a list of specific `scheduleIds` to run now.
