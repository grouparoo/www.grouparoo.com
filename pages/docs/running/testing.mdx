...
- You need to specify to Grouparoo which database to use, `server_token` to use, etc. You can either define these directly in your test, as environment variables on your CI tool, or tell Grouparoo to load a `.env` file directly by setting `process.env.GROUPAROO_ENV_CONFIG_FILE`. With either of these methods, this needs to be done before requiring `@grouparoo/spec-helper`.
