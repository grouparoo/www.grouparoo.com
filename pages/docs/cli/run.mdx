...
- The `grouparoo run` command will run _all_ schedules exactly once. We will not continue to add new runs as time progresses per a Schedule. You can choose to run non-recurring schedules as well via a flag.

<CLICommand name="start" />

The `grouparoo start` command is equivalent to `npm start`.

<CLICommand name="reset" />
