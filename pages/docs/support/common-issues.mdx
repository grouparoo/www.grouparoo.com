...
In general, a Run will wait for all of its Imports and Exports to complete before for entering the "complete" state themselves. A Run will still complete if it encounters an error when trying to import or export a Record, but not _trying_ to do the import or export will mean the run is still pending.
