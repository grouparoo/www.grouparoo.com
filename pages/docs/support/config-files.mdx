...
Code config files can use JavaScript functions that can export an `async` method, which can `await` while loading data from a database or API. (See the _Examples_ section below.)
