...
Grouparoo projects contain configuration files for Apps, Models, Sources, Properties, Groups, Destinations, and any other objects is exported into config files.
...
- `type`: (**required**) The type of the app define by a plugin. While the type can match the name of the plugin, plugins may offer multiple apps and the names may not match.
