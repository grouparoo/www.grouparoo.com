...
The old behavior may have lead to unintended removal of Records from your Destinations in certain SyncModes. If you do want to remove the previously exported Records from a destination, choose to sync `"none"` in combination with SyncMode `"sync"` after previously exporting Records.
