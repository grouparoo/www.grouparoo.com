...
Google Sheets now needs the aggregation method explicitly declared. While it's always only supported `exact` Properties, the following will need to be added to your Property config files if you use a Google Sheets source:
...
As of Grouparoo v0.8, Manual Groups are no longer supported. All groups are now configured based upon sets of Group Rules, rather than by manually adding Records. As such, there is no longer any group "type". You will want to remove the following line from your Group config files, even for calculated groups:
