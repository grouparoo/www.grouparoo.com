...
The full set of options is: `REDIS_URL="postgresql://{user}:{password}@{host}:{port}/{database}?{ssl=true}"`. You can also modify the SSL connection options to your postgres database with these environment variables:
