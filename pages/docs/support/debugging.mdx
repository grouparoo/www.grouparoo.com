...
Application usage refers to the size of the data you are processing with Grouparoo. Are there more Records or Imports than normal? Is your database filling up? Check these by connecting to each database in the Data Tier directly and reporting its usage metrics, like number of rows, slow queries, etc.
...
Can you break things in the same way multiple times? Reproducible bugs are possible to fix, and it’s important to explain why they happen. Does visiting a certain page crash things? Does importing or exporting a certain Record cause an error?

Sharing this information to reproduce the problem is important when [reporting a bug](/docs/support).
