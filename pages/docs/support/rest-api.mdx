...
# Make Request
curl -X POST "$GROUPAROO_HOST/api/v1/source?appId=$APP_ID&name=$SOURCE_NAME&options=%7B%22table%22%3A%20%22$TABLE_NAME%22%7D&type=$SOURCE_TYPE&state=ready&apiKey=$GROUPAROO_API_KEY"
```
...
SOURCE_ID="src_22d32d1d-5573-4e1d-a4a7-151d8329e4cb"
ID="email"
key="email"
options="=%7B%22column%22%3A%22email%22%2C%20%22aggregationMethod%22%3A%22exact%22%7D" #these are an encoded object
type="email"

# Make Request
curl -X POST "http://localhost:3000/api/v1/property?id=$ID&key=$KEY&options=$OPTIONS&sourceId=$SOURCE_ID&state=ready&type=$TYPE&apiKey=$API_KEY"
```
