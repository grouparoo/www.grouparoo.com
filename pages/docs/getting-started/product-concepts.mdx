...
[Destinations](#destination) have a Sync Mode, which tell the Grouparoo application how to create and delete Records on this Destination. The types of Sync Modes are:

- **Append** (`sync`): Always create new records, even if they already exist in the Destination.
- **Create** (`sync`): Create new records if they don‘t already exist in the Destination.
