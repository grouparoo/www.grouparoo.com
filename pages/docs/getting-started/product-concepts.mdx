...
Groups are a collection of [Records](#record) that you can define and Filter. These filters (or _rules_) are built to be easy-to-understand by non-technical users.
...
A Schedule defines some interval by which to process a [Run](#run). Schedules define a frequency that Grouparoo should check your [Sources](#source) for new data. Whenever possible, Schedules use [High watermarks](#high-watermark) to scan for only new and updated data in your Data Warehouse. Schedules can also be triggered manually or via the [API](/docs/running/elt-integration#triggering-schedules-via-the-api).
