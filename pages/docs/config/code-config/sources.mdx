...
The very first Source you create in Grouparoo will need a [Property](/docs/config/code-config/properties) to use as a primary key for each Profile. This must be unique to each of your Profiles, and is commonly a user ID or email address. Grouparoo will generate one Profile per unique value for this Property.
