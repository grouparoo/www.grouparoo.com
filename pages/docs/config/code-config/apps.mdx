...
If your app supports an App Refresh Query, you'll see a field with a `refreshQuery` object. App Refresh Query enables Grouparoo to know where to check your app for new data, and run all Schedules for that App if it finds new data. For example, you might set it up as:
