...
## Validating and Applying Config

The `generate` command isn't the only useful CLI tool while build out your configuration. Both the [`validate`](/docs/cli/config#validate) and [`apply`](/docs/cli/config#apply) commands come in handy.

`validate` ensures that you're code config is of the proper shape and that connections are valid. `apply` will actually write your configuration to the application database.
