...
Your Property config file will also contain `aggregationMethod` and `filter` options. Options for these may vary depending on your source type. All available options for your source will be listed in your config file template created when you grouparoo generate. Some common examples are:
