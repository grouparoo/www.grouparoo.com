...
image: "integrating-mailchimp-with-your-nodejs-app/mailchimp_node.png"
tags: [connections, engineering]
---

# Integrating MailChimp with your Node.js App

If you’re reading this blog right now, chances are that you’re finding your way to add a MailChimp integration into your Node.js application. Don’t worry, we have you covered! This is a step by step guide which will help you build a sample Node.js backend for your webpage and help you integrate it with the MailChimp API. 

In this tutorial, we will talk about all the different things you need to look for within MailChimp with references to their awesome API documentation, talk about a few edge cases you need to consider and finally talk about an alternative method using Grouparoo which will do all the heavy lifting for you!
...
Setting up your MailChimp account is a fairly simple process. In this tutorial we’re assuming that you have created a free MailChimp account and are looking to integrate it within your application. For premium accounts, the process is fairly similar, just that you have the ability to create multiple audiences, each having their separate Audience ID.
To integrate your app with Mailchimp, we need to pieces of information from your account:
...
For integrating your app, the first and most important thing needed is an API Key. This is a fairly easy step, just have a look at the steps below.
...
/>
4. Once you click on it, you’ll see that mailchimp had created an API key for you! It will look something similar to this hereWe have displayed the later portion of our API Key here. One thing to note here are the last 3 characters after the - data center corresponding to your account. Each account has its own corresponding data center for all the API calls one can make. For us, it is us6. This might be totally different for you, make sure to note this separately
...
For this tutorial, we’re assuming that you have a sample frontend application, where users can easily enter their email and click on the signup button to register on your mailing list.
...
All the error messages are being logged into the browser console.

Now, when we have set up our frontend, it is time to create our backend

In your repository, it is time to create a package.json file to capture all the necessary modules required by our project
...
Now it is time to install all our necessary modules for the project. We will be requiring the following modules:
express:
body-parser:
request:
...
2. In order to parse the data that is coming from the POST request, we will be using body-parser
```
...
3. Next up, we need to specify the folder which contains your frontend application and connect it to the express server. Here, we’re having a folder named `public`, which contains all our frontend code.

```
app.use(express.static(path.join(__dirname, 'public')))
```

4. Perfect, let’s create our POST request quickly. We’re just setting it up so just adding a console log to check if everything’s working as expected.
...
5. Now let’s define the PORT on which our application will hosted

```
const PORT = process.env.PORT || 8888;
```

This sets the value to the environment PORT value in case we’re using a cloud hosting or default it to 5000 which is our local machine endpoint.
...
7. Woof! That's a lot of setup for just a basic API request. Let’s test our setup uptill now. We’ll be using the node command for this
...
You can also go and check your frontend for starting a post request by entering in the email field. As soon as you do that, you’ll notice the terminal says
```
I am listening
```

This proves that our server is up and running and the post request has also gone through. In case you do not have a frontend and want to test this, you can easily use a tool like postman and send a post request similar to the one we have mentioned above for the frontend application.

Now finally, it is time to do some actual fun stuff! Let’s integrate this with MailChimp!

## Connecting your Node.js App to MailChimp

For this whole process, we’re directly referring to the [MailChimp API Documentation](https://mailchimp.com/developer/marketing/api/list-activity/). They have excellent documentation stating all the different things you need to be aware of with extremely detailed resources available. Make sure to check that out in case you need any additional information that is not mentioned here directly.

We’re using the standard URL route here, if you want to download the [MailChimp NodeJS module](https://www.npmjs.com/package/mailchimp-api-v3), you can definitely do that as well. Although it makes your life easier in terms of implementing the API, it definitely adds a bunch of other things which we do not need for this particular project. 
...
So currently as we can see from our earlier setup that our server is listening to `/subscribe` but it is not doing anything actually other than just letting us know in the console that it is working. Now, we’ll be grabbing the request from here and directing it later to the MailChimp API.

So, let’s grab our email from the request really quickly.
...
And that’s it! We’re totally done with the code here! You have successfully made a way for people to opt in for your mailing list!
