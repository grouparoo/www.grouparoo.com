---
title: "Asynchronously Exporting Data to Remote Systems"
date: "2021-05-27"
author: "Pedro S Lopez"
pullQuote: "Some batch import APIs work asynchronously and don't process your requests right away."
image: "asynchronously-exporting-data-to-remote-systems/bus-stop.jpeg"
tags: [engineering]
---

When integrating with destinations, there are generally two main approaches made available by API providers: **single** or **batched**. With the "single" approach, one API request usually affects a single profile in the destination. The "batched" approach, which you can read more about [here](https://www.grouparoo.com/blog/batching-api-requests), allows you to affect multiple profiles in a single API request. This is usually preferred due to rate limits put in place and because they are often faster than their single counterparts when dealing with a large number of profiles.

However, within batched APIs you may find a few different approaches. In most cases, issuing an API request to import a batch of profiles to a destination resolves right away and immediately gives a response with information about whether the action was successful or not, as well as which profiles had errors. On the other side, some APIs enqueue the action to be processed later and instead require you to check back on its status to know if it was successful.
...
Similarly, [SendGrid](https://sendgrid.api-docs.io/v3.0/contacts/add-or-update-a-contact) can also work like this, though it's a bit more streamlined and only requires a single request to set up the import. The result of this request, much like Eloqua, includes a `job_id` that you can use to check on the status of the import.

### Size limits

Something to note about these imports is that they usually have a limit on the size of the request that can be issued. For example, Eloqua has a hard limit of 32MB per request, though it allows you to issue multiple upload requests up to a recommended 250MB in total. SendGrid is a bit more limiting, only allowing 6MB of data to be sent per job.
...
As a tool to keep your data synchronized between systems, Grouparoo has all of this built-in. Take a look at our available integrations, or make your own plugin to leverage the platform. Best of all, we're open source!

<Image
  alt="Waiting at the bus stop"
  src="asynchronously-exporting-data-to-remote-systems/bus-stop.jpeg"
