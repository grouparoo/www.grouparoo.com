...
<Image
  width={"100%"}
  alt="Share Dataset"
  src={require("../../public/posts/typescript-frontend-backend/screenshot-1.png")}
/>
...
Now we have enforced that the type of `response` in the `load()` method above will match the Action, being `{ time: number; }`. We will now get help from Typescript if don’t use that response value as a number, if we try to assign it to a string variable or otherwise use it improperly.

<Image
  width={"100%"}
  alt="Share Dataset"
  src={require("../../public/posts/typescript-frontend-backend/screenshot-2.png")}
/>
...
If this type of work is interesting to you, [Grouparoo is hiring](www.grouparoo.com/jobs)!
