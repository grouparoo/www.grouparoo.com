...
Recently, working on the Grouparoo source plugin for MongoDB, I came across a very simple requirement (simple, at least on SQL-based DBs with fixed schemas) which is to get all field names of a collection. However, this is not a simple query on MongoDB as it should be and we’re going to see what is needed to achieve this.
...
```
db.runCommand({
 "mapreduce": "my_collection", "map": function() {
   for (var key in this) {
     emit(key, null);
   }
...
After that, run the distinct command on the resulting collection to get the a set containing all keys:
...
There are several other ways to achieve the same result, but the aggregation pipeline solution provides better performance (at least than the Map-reduce solution) and usability than the other ones.
