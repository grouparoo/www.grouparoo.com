...
Teams are centralizing their data in their data warehouse by loading data in and transforming it as necessary. Increasingly, we are seeing teams turn to [dbt](https://www.getdbt.com) to do this transforming. The idea is to write \*.sql files that, when run in the right order, creates useful rollup tables or materialized views of the data.
