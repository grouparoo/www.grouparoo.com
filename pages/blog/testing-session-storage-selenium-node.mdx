...
date: "2021-02-18"
author: "Sean C Davis"
pullQuote: "How to test for key-value pairs in sessionStorage and localStorage using Selenium Node."
image: testing-session-storage-selenium-node/selenium-session.png
tags:
  - engineering
  - notes
---

We have [a feature on this site that is using `sessionStorage`](/blog/getting-previous-path-nextjs) to send analytics data we want to capture. It's an important feature and so we looked to do what we _should_ — write test(s) to cover the use case(s).
...
We can use the `executeScript` method to run a JavaScript expression and capture the result. [Our test looks like this](https://github.com/grouparoo/www.grouparoo.com/blob/master/__tests__/integration/sessionStorage.ts):
...
- `browser` is often referred to as `driver` in other documentation and implementations. Yours may differ.
