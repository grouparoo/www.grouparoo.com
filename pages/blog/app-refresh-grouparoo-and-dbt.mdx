...
Grouparoo’s new App Refresh feature enables users to run their Grouparoo schedules whenever a predetermined query returns new data. This means you can do things like update your Users and Purchases data anytime your Payments table is updated or run all of your Schedules every time a dbt job completes.

We’ve talked about how to [integrate dbt with Grouparoo](/blog/dbt-and-grouparoo) in your Reverse ETL process before. One use case that was important for us to support is how to integrate Grouparoo into an existing dbt project and trigger schedules automatically when a dbt run completes -- including for users on our Hosted offering. Now, you’re able to run Grouparoo after your dbt runs in any environment!
