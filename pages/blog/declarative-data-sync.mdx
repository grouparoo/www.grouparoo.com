...
pullQuote: "Grouparoo supports declarative data models and integrations to sync data to cloud-based tools. Reverse ETl becomes part of your standard dev stack."
...
- Generate an [App](https://www.grouparoo.com/docs/config/code-config/apps) with connection information (Postgres database, etc).
- Generate a [Source](https://www.grouparoo.com/docs/config/code-config/sources) with [Properties](https://www.grouparoo.com/docs/config/code-config/properties) (id, email, first_name from users table) to create Profiles.
- Generate calculated [Groups](https://www.grouparoo.com/docs/config/code-config/groups) of Profiles (High Value Users) based on Profile Property values.
- Generate a [Destination](https://www.grouparoo.com/docs/config/code-config/destinations) and map the data to it (sync email, first_name, and group membership to Mailchimp)
