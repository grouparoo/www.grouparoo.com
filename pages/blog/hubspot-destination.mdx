...
From there, you can create a Destination and start exporting your Profiles and Groups. You can learn more about [creating Destinations here](/docs/guides/create-destinations)
...
... you get the idea. You can see how we implemented this our code [here](https://github.com/grouparoo/grouparoo/blob/master/plugins/@grouparoo/hubspot/src/lib/export/destinationMappingOptions.ts).
...
To solve this, we needed to introduce a distributed lock across all nodes in the Grouparoo cluster when creating or destroying Hubspot lists. This means that our [`getLists()` method](https://github.com/grouparoo/grouparoo/blob/master/plugins/@grouparoo/hubspot/src/lib/export/listMethods.ts) needed a way to "claim" the hubspot API exclusively from any other process or thead while it checks if a list existed or not. [Redis](https://redis.io) is already part of the Grouparoo infrastructure, and is a great backend for shared locks like this. One process can claim a lock and be certain that they have it, due to the single-threaded atomic nature of redis. Redis also provides APIs to have locks time out after a certain time if anything goes wrong with the API request, so another process can try again. Redis is also [very fast](https://www.digitalocean.com/community/tutorials/how-to-perform-redis-benchmark-tests).
